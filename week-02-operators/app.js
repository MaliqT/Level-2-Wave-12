...
const gameName = "Space Blaster";   // Title of game does not change once launched
let playerName = "Maliq T";       // Most games and even client accounts allow for name change
let playerScore = 0;                // player score changes over the course of the game
let highScore = 850;                // player high score changes if the player achieves a higher score than their previous
const pointsPerKill = 25;           // points for getting a kill doesn't change 
let livesRemaining = 3;             // When the player dies, the value goes down
...
const totalEarned = 6 * pointsPerKill;
...
console.log(livesRemaining > 0);    // true/false means if the players remaining lives is greater than 0
...
console.log(playerScore > highScore);   // false since highScore is 850, 225 is less than 850
console.log(playerScore === highScore); // false, we're using strict equality. Both types are the same but the values aren't
console.log(playerScore >= highScore);  // Still false. playerScore is still less than highScore
...
//highScore = playerScore;   The high score is still higher than current player score
...
console.log(accuracyPercent >= 75);     // We can use .toFixed which returns a string represented in fixed-point notation
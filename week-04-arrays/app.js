...
const studentNames = ["Joe", "Sam", "Justin", "Jared", "Alan", "Chris"];
const studentScores = [74, 58, 92, 65, 81, 47];

// I'm using const for everything because class name won't change and neither will passing score since that is the fixed score to pass
// I'm using const for both arrays so that their variable reference is locked 
...
console.log(`📊 Class Stats:
    Average score: ${classAverage.toFixed(2)}
...
let removed = studentNames.shift();
studentScores.shift();
...
console.log(`Original scores: ${studentScores}
    Curved scores: ${curvedScores}`);

let newTotalScores = 0;
let newHighScore = 0;
let newLowestScore = curvedScores[0];

for (let i = 0; i < curvedScores.length; i++) {
    newTotalScores += curvedScores[i];
    
    if (curvedScores[i] > newHighScore) {
        newHighScore = curvedScores[i]
    }
    if (curvedScores[i] < newLowestScore) {
        newLowestScore = curvedScores[i]
    }
...
const newClassAverage = newTotalScores / curvedScores.length;

console.log(`📊 Class Stats:
    Average score: ${newClassAverage.toFixed(2)}
    Highest score: ${newHighScore}
    Lowest score: ${newLowestScore}
    Passed: ${passCount} | Failed: ${failCount}`);
...
const inventory = [
    {id: 1, name: "Lightbulb", category: "Electronics", price: 19.99, stock: 15, isAvailable: true},
    {id: 2, name: "Charger", category: "Electronics", price: 8.99, stock: 3, isAvailable: true},
    {id: 3, name: "Banana", category: "Food", price: 7.99, stock: 0, isAvailable: false},
    {id: 4, name: "Apple", category: "Food", price: 150.99, stock: 1, isAvailable: true},
    {id: 5, name: "Towel", category: "Bathroom", price: 16.99, stock: 18, isAvailable: true}
]
...
for (const item in inventory) {
    console.log(`#${inventory[item].id} ${inventory[item].name} | $${inventory[item].price} | Stock: ${inventory[item].stock} | (${inventory[item].isAvailable ? "Available" : "Out of stock"})`);
}
...
for (const item in inventory) {
    totalValue += inventory[item].price * inventory[item].stock;
    totalItems += inventory[item].stock;

    if (inventory[item].stock === 0) {
        outOfStockCount++;
    } else if (inventory[item].stock < lowStockThreshold) {
        lowStockCount++;
    }
}
...
console.log(`Total inventory value: $${totalValue.toFixed(2)}
    Total items in stock: ${totalItems}
    Out of stock: ${outOfStockCount} product(s)
    Low stock: ${lowStockCount} product(s)`);
...
for (const item in inventory) {
    if (inventory[item].category === searchCategory) {
        found++;
        console.log(`🔍 ${inventory[item].name} - $${inventory[item].price}`);
    }
}
...
for (const item in inventory) {
    if (inventory[item].category === "Electronics") {
        inventory[item].price -= inventory[item].price * discountRate;
        inventory[item].price = Math.round(inventory[item].price * 100) / 100;

        console.log(`${inventory[item].name} discounted to $${inventory[item].price}`);
    }
}
...
for (const item in inventory) {
    if (inventory[item].stock === 0) {
        inventory[item].stock += 50;
        inventory[item].isAvailable = true;
        
        console.log(`Emergency restock: ${inventory[item].name} → ${inventory[item].stock} units`);
    } else if (inventory[item].stock < 5) {
        inventory[item].stock += 20;
        console.log(`Restock: ${inventory[item].name} → ${inventory[item].stock} units`);
    }
}
...
const supplyElectronics = {name: "PowerRUs", country: "Europse"};
const supplyFood = {name: "Farmers Delight", country: "USA"};
const supplyBathroom = {name: "Body & Bath", country: "Italy"};

for (const item in inventory) {
    if (inventory[item].category === "Electronics") {
        inventory[item].supplier = supplyElectronics;
    } else if (inventory[item].category === "Food") {
        inventory[item].supplier = supplyFood;
    } else if (inventory[item].category === "Bathroom") {
        inventory[item].supplier = supplyBathroom;
    }

    console.log(`${inventory[item].name} supplied by ${inventory[item].supplier.name} (${inventory[item].supplier.country})`);
}

// Was curious to see if I could write everything for this task inside a single for loop instead of individually adding supplier to each item object.
// So I wrote it here.
...
for (const item in inventory) {
    currentValue = inventory[item].price * inventory[item].stock;
...
const topItem = inventory[topValueIndex]
console.log(`Most valuable: ${topItem.name} | $${topItem.price} x ${topItem.stock} units = $${topItem.price * topItem.stock}`);
...
const categories = ["Electronics", "Clothing", "Food", "Bathroom"];
...
for (const category in categories) {
    ...
    for (const item in inventory) {
        if (inventory[item].category === categories[category]) {
            productCount++;
    ...
    console.log(`--- ${categories[category]} ---
        ...
        Category value: $${totalValue.toFixed(2)}`);
}
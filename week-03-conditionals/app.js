...
if (isExtraCredit) {
    ...
    console.log(`Extra credit applied! Your new score is: ${earnedScore}`);
} else {
    console.log(`No extra credit applied. Sorry. Current score is: ${earnedScore}`);
}
...
// 90 comes before 80 because if we got a score of 94, then 80 would get checked first which is true and Grade B will be printed which is not true. Grade A should be printed.
// It should also be noted that the order matters depending on which direction we are checking. 
// Let's say we were checking if percentage is lower, well if we get a score of 70, then 90 would get checked first and still pass so Grade A will be printed. So in this case, we must
// flip the order of conditions.

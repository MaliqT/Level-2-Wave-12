...
console.log(createUser("MaliqT", "MaliqT@email.com", 28, false));
console.log(createUser("Joe", "joe@email.com", 22, false));

//isPremium = false is a good default because we want users to start out as false premium users first until they purchase premium or until confirmed they are premium users
...
const isValidUser = function (user) {
  return user.username.length > 0 && user.email.length > 0 && user.age >= 13 ? true : false;
}
...
//isValidUser is a function expression because we are returning a value from the function and assigning it to a variable so that the result can be used again in either another function or data structure.
...
//An arrow function is a good fit here because it's a one-liner function and we're also using it inside an array method
...
console.log(`Count: ${youngAdults.length}`);
console.log(youngAdults.map(user => user.username));

console.log(`Count: ${teens.length}`);
